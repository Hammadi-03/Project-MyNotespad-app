...
@section('content')
<style>
    :root{--primary:#4F46E5;--primary-hover:#4338CA;--primary-light:#EEF2FF;--danger:#EF4444;--gray-50:#F9FAFB;--gray-100:#F3F4F6;--gray-200:#E5E7EB;--gray-400:#9CA3AF;--gray-500:#6B7280;--gray-600:#4B5563;--gray-700:#374151;--gray-800:#1F2937;--gray-900:#111827;--sidebar-w:260px;--topbar-h:64px;--radius:12px;--radius-sm:8px;}
    ...
    @media(max-width:640px){
        ...
        .editor-card-top{padding:24px 20px 0;}
    ...
    <div class="editor-container">
        <div class="editor-card">
            <form id="noteForm" method="POST" action="{{ route('notes.store') }}">
                @csrf
                <input type="hidden" name="color" id="selectedColor" value="{{ old('color', 'default') }}">
                ...
                <textarea
                    ...
                    class="content-area"
                    id="contentArea"
                    placeholder="Start writing your note...&#10;&#10;Your ideas deserve a great home 💡"
                >{{ old('content') }}</textarea>

                <div class="options-bar">
                    <span class="options-label">🎨 Color:</span>
                    <div class="color-picker">
                        <div class="color-swatch {{ old('color','default')==='default' ? 'selected' : '' }}" data-color="default" style="background:#E5E7EB;" title="Default" onclick="selectColor(this)"></div>
                        <div class="color-swatch {{ old('color')==='blue' ? 'selected' : '' }}"    data-color="blue"    style="background:#3B82F6;" title="Blue"    onclick="selectColor(this)"></div>
                        <div class="color-swatch {{ old('color')==='green' ? 'selected' : '' }}"   data-color="green"   style="background:#10B981;" title="Green"   onclick="selectColor(this)"></div>
                        <div class="color-swatch {{ old('color')==='yellow' ? 'selected' : '' }}"  data-color="yellow"  style="background:#F59E0B;" title="Yellow"  onclick="selectColor(this)"></div>
                        <div class="color-swatch {{ old('color')==='red' ? 'selected' : '' }}"     data-color="red"     style="background:#EF4444;" title="Red"     onclick="selectColor(this)"></div>
                        <div class="color-swatch {{ old('color')==='purple' ? 'selected' : '' }}"  data-color="purple"  style="background:#8B5CF6;" title="Purple"  onclick="selectColor(this)"></div>
                    </div>
